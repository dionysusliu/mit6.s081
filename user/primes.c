...
void emitter_proc() {
    ...
    for (int n = 3; n < 35; n++) {
        ...
        if (!have_child) {
            if ((child_pid = fork()) == 0) { // create child process
                close(p[1]); // child doesn't use this write portal
                filter_proc(p[0], n);
            }
            have_child = 1;
        }
    ...
    wait((int *)0);
    debug_printf("++emitter: waiting for child processes to finish\n");
    exit(0);
}

__attribute__((noreturn))
void filter_proc(int in_pipe, int div) {
    printf("prime %d\n", div);
    int p[2];
    ...
    while((n = read(in_pipe, &candidate, sizeof(candidate))) != 0) {
        if (n < 0) {
            fprintf(2, "filter_proc: read error\n");
            exit(1);
        }
        debug_printf("filter [%d] read %d bytes from in_pipe {%d}\n", div, n, in_pipe);
    ...
    debug_printf("-- filter [%d] stop reading from pipe", div);
    close(in_pipe); // clean the in_pipe
